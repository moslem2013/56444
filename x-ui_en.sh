...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/moslem2013/56444/master/install_en.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/moslem2013/56444/raw/master/x-ui_en.sh
    if [[ $? != 0 ]]; then
        echo ""
